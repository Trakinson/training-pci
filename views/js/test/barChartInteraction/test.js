...
define([
    ...
    'text!trainingPci/test/barChartInteraction/data/oneChart/qti.xml'
], function ($, _, url, assetManagerFactory, assetStrategies, portableAssetStrategy, qtiItemRunner, ciRegistry, pciTestProvider, oneChartXml) {
     ...
     */
    pciTestProvider.addManifestPath('barChartInteraction', 'trainingPci/pciCreator/barChartInteraction/pciCreator.json');
    ...
    QUnit.asyncTest('renders correctly', function (assert) {
        ...
        parseXml(oneChartXml).then(function (itemData) {
            var assetManager = getAssetManager('/trainingPci/views/js/test/barChartInteraction/data/oneChart/');
            var runner = qtiItemRunner('qti', itemData, {assetManager: assetManager})
                .on('render', function () {
                    ...
                    assert.equal($container.find('.qti-customInteraction .barChartInteraction').length, 1, 'the container contains 3 likert interactions');
    ...
    QUnit.test('display and play', function (assert) {
        ...
        parseXml(oneChartXml).then(function (itemData) {
            var assetManager = getAssetManager('/trainingPci/views/js/test/pbarChartInteraction/data/oneChart/');
